...
if (isset($_POST['fetchBranch']) &&  $_POST['fetchBranch'] === 'fetch-branch') {
    ...
    if (isset($_POST['sortIDBranch']) && isset($_POST['sortDateBranch'])) {
        if (($_POST['sortIDBranch'] === '' || $_POST['sortIDBranch'] === 'ASC' || $_POST['sortIDBranch'] === 'DESC') &&
            ...
            if ($_POST['sortDateBranch'] !== '' && $_POST['sortIDBranch'] !== '') {
                $sortIDBranch = $_POST['sortIDBranch'];
                $sortDateBranch = '';
                ...
                $sortIDBranch = '';
                $sortDateBranch = $_POST['sortDateBranch'];
            } else if ($_POST['sortDateBranch'] === '' && $_POST['sortIDBranch'] === '') {
                $sortIDBranch = 'ASC';
                $sortDateBranch = '';
            }
    ...
    $dataBranch =  fetchBranch(
        $ValidateData->standardizeString($limitBranch),
        $ValidateData->standardizeString($startBranch),
        $ValidateData->standardizeString($queryBranch),
        $ValidateData->standardizeString($sortIDBranch),
...
if (isset($_POST['createBranch']) && $_POST['createBranch'] === 'create-branch') {
    ...
    try {
        ...
        $BranchClass = new Branch();
        $result = $BranchClass->insertBranch($conn); // Truyền kết nối vào hàm insertEmployee

        if ($result['status'] === 'success') {
            echo json_encode(['status' => 'success', 'id' => $result['id']]);
        } else {
            error_log("Lỗi thêm chi nhánh: " . $result['error']); // Ghi log lỗi
...
//     try {
//         $conn = new PDO("mysql:host=localhost;dbname=projectweb2", "root", "");
//         $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

//         $BranchClass = new Branch();
//         $result = $BranchClass->insertBranch($conn); // Truyền kết nối vào hàm insertBranch
